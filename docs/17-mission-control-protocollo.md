...
## Aggiornamento 28/8 notte — digest per il CAPO e contatori Scout live
- Nuovo GET /api/ingest?digest=1 (stessa chiave ingest): il quadro completo in una chiamata per il CAPO: agents (stato e ultimo giro), ultimi_giri (con le checklist CHK nei summary), pipeline_per_stage, creators con esito, bozze_per_stato + bozze_in_attesa, kv. E' l'UNICA fonte numeri ammessa per il report del CAPO (insieme alle letture Airtable fatte nel giro stesso).
- kv scout_stats = contatori live della tabella Leads ({tot, pronto, da_arricchire, contattato, scartato, updated_at, fonte}): la pagina Scout e la home li leggono al posto dello snapshot impacchettato. Li mantiene il CAPO applicando i numeri della checklist dello SCOUT (tot += nuovi, pronto += pronto_nuovi, scartato += scartati, da_arricchire = resta_arricchire). Seed del 28/8 sera coi numeri contati: 2848 / 32 / 4 / 0 / 2812.
- CAPO v2 (trig_01YLHqspVmREVr853DmvfGKx): formato report fisso 7 righe, UNA sola entry feed per giro (il run_finish pubblica gia' il summary: vietata la op feed doppia), karma SOLO comment_karma, controllo stalli degli altri agenti, verifica a campione dashboard vs record Airtable giusto.
