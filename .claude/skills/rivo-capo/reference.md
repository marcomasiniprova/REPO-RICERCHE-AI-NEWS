...
- VIDEO: "CHK saldo_kie= crediti_spesi= generato= rigenerazioni= qa= stato= pubblicato_arretrato= piattaforme= attesa_pin_min=". Campanelli: saldo_kie basso o a zero (la macchina contenuti si ferma: priorita' con destinatario Valerio, "ricarica Kie"), qa fallito due giorni di fila (problema di regia: dillo al builder), stato in_attesa da oltre 24h (il PIN manca: in cima alle priorita'), pubblicato_arretrato>0 (bene: un PIN ha sbloccato un video). I video vivono nel kv (chiavi video_YYYY-MM-DD): stato e date le leggi da li'.
