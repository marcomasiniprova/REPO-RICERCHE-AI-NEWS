...
- VIDEO: "CHK saldo_kie= crediti_spesi= generato= rigenerazioni= qa= stato= pubblicato_arretrato= piattaforme= attesa_pin_min=". Campanelli: saldo_kie basso o a zero (la macchina contenuti si ferma: priorita' con destinatario Valerio, "ricarica Kie"), qa fallito due giorni di fila (problema di regia: dillo al builder), stato in_attesa da oltre 24h (manca l'approvazione: in cima alle priorita'), pubblicato_arretrato>0 (bene: un'approvazione ha sbloccato un video). I video vivono nel kv (chiavi video_YYYY-MM-DD): stato e date le leggi da li'.
...
- Una priorita' e' un'AZIONE con un DESTINATARIO, non un desiderio. Si': "approvazione alla bozza di Yass (scade lo slot di sabato)". No: "migliorare le conversioni".
...
- Le bozze in attesa di approvazione non sono un guasto: sono Valerio che deve decidere. Si scrive "aspettano l'approvazione", con la piu' vecchia in evidenza, e se superano le 24h diventano priorita' 1.
