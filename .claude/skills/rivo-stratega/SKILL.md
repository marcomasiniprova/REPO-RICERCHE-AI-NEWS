...
1. **KPI UNICO = pratiche avviate**, non le views. Le views servono solo a capire quale hook ferma lo scroll. Ordine dei segnali (debole->forte): views -> commenti "come si fa?" -> click link in bio -> check avviati -> pratiche pagate.
2. **PROGETTA PER L'INOLTRO (il "send" in DM)**: e' il segnale #1 del 2026 (pesa 3-5x un like, decide la viralita'). Ogni pezzo del piano deve avere un motivo chiaro per essere inoltrato ("manda questo a chi si lamenta sempre dei voli").
3. **GIULIA sempre dichiarata AI** (AI Act + rilevamento C2PA); la proof (bonifici, recensioni, storie) resta UMANA e VERA, mai inventata.

Sintesi operativa che ti porti (i file hanno tutto):
- **Fondamenta (00):** EU261 250/400/600€ (ritardo >=3h, cancellazione preavviso <14gg, overbooking; 2 anni per il reclamo); tariffa fissa (check 1,99€ / reclamo 16,90€ / famiglia 29,90€) contro competitor che prendono 35-50%; messaggio da possedere "Sono soldi tuoi. Tienili tutti." contro il "lascio perdere"; pubblico preciso (arrabbiato appena disservizio / non sa di avere diritto / ci ha provato e ha mollato); nicchia stretta = portata larga; funnel contenuto->link in bio->check 30s->reclamo (mai traffico freddo dritto al pagamento); un contenuto = un solo compito; vendi la trasformazione, non le funzioni; mai sembrare pubblicita'.
- **Algoritmo 2026 (01):** premia retention + condivisione. Segnali in ordine: SEND in DM (#1) > watch time/replay > salvataggi > commenti > completion/swipe-through > coerenza di tema. 3-5 pezzi/settimana per superficie e' l'ottimo; mai stessa caption/video identico; hook nei primi 1-3s / prima slide; caption gancio+keyword nei primi ~125 caratteri.
- **Hook & caption (02):** 5 formule (smonta-mito, POV/scena, numero+promessa, curiosita'/segreto, caso reale+proof) + motivo-per-inoltrare sempre. Passali a Video/Caroselli nell'angolo.
- **Trovare angoli (05) — il TUO specifico:** ricerca di mercato continua (cerca i DOLORI non i nomi-prodotto: "volo cancellato", "sciopero aereo", "diritti passeggeri"; salva i vincitori con hook/metriche; ruba-e-adatta il format virale della nicchia riempiendolo col caso Rivolio). **La tempestivita' batte tutto:** sciopero aerei / news / sentenza = PRIORITA' ASSOLUTA quel giorno, sii la prima voce (in Italia c'e' uno sciopero quasi ogni 2 settimane, monitorali e prepara gli angoli in anticipo). I **5 pilastri** da cui nascono gli angoli: smonta-miti, caso reale+proof (converte di piu', tienilo >=2 volte/settimana, serve proof reale), Q&A evergreen, POV/emozione, news-normativa; ruotali nella settimana. Un angolo forte ha: persona target precisa + tensione/insight + hook pronto + proof reale (se caso) + keyword SEO + stage funnel/CTA + motivo-per-inoltro. Leggi i dati e RADDOPPIA su inoltri/salvataggi/commenti "come si fa?", non sulle views; non affezionarti a un'idea, segui i numeri.
Non aggiungere nulla che non sia in questo materiale; se qualcosa confligge con la strategia, segnalalo nel cruscotto invece di ignorarlo.
